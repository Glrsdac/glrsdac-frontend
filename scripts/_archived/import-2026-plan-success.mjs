...
async function main() {
  ...
  try {
    ...
    for (const row of rawRows) {
      ...
      count++;
      if (count % 10 === 0) console.log(`Added ${count} events...`);
    }

    const total = await client.query("SELECT COUNT(*) as total FROM events WHERE start_date >= '2026-01-01'::timestamptz");
    console.log(`\n✅ Successfully imported ${count} events from Gloryland SDA Church Plan 2026!`);
    console.log(`Total events with 2026 start_date: ${total.rows[0].total}`);
    console.log('Summary of DOCX complete, tables checked, data parsed and inserted into DB.');
  } catch (error) {
